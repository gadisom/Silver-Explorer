...
class ARKioskForCreditCard: ARKioskModel {
    
    private var updatedScale: CGFloat = 0.0
    
    init(containerNode: SCNNode) {
        super.init(kioskContainerNode: containerNode, kioskNodeName: String(describing: ARKioskForCreditCard.self))
    }
    
    static func makeContainerNode() -> SCNNode? {
        let node: SCNNode = SCNNode()

        guard let arKioskScene = SCNScene(named: String(describing: ARKioskForCreditCard.self) + ".scn") else {
            print("Hi")
            return nil
        }
        guard let arKioskNode = arKioskScene.rootNode.childNode(withName: String(describing: ARKioskForCreditCard.self), recursively: true) else {
            print("Yes")
            return nil
        }
